...
class Settings(BaseSettings):
    ...
    IMAGE_DPI: int = 1024 # DPI to render images pulled from pdf at
    ...
    # Texify model
    TEXIFY_MODEL_MAX: int = 1024 # Max inference length for texify
    TEXIFY_TOKEN_BUFFER: int = 256 # Number of tokens to buffer above max for texify
    TEXIFY_DPI: int = 1024 # DPI to render images at
    TEXIFY_BATCH_SIZE: Optional[int] = None # Defaults to 6 for cuda, 12 otherwise
    TEXIFY_MODEL_NAME: str = "vikp/texify"

    # Layout model
    SURYA_LAYOUT_DPI: int = 512
    BAD_SPAN_TYPES: List[str] = ["Page-footer", "Page-header", "Picture", "Footnote"] # You can add "Caption" and "Footnote" here to get rid of those elements - this just removes the text, not the image in case of Picture
    LAYOUT_MODEL_CHECKPOINT: str = "vikp/surya_layout3"
    BBOX_INTERSECTION_THRESH: float = 0.8 # How much the layout and pdf bboxes need to overlap to be the same
