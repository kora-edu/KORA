...
def load_data():
     ...
     Settings.llm = OpenAI(model=gpt_model, temperatue=0.5, system_prompt=f"You are a helpful and intelligent tutor that has the capability to describe {subject} in all ranges of levels of understanding")
...
if selected_sIndex !=0 and selected_YrIndex !=0 and uploaded_file:
    but_disabled = False
    gen_button = st.button('Generate question', on_click=begin_conversation, type="primary", disabled=but_disabled)
    ...
    # st.subheader('Generated Exam Problems:')
    # st.write(response)